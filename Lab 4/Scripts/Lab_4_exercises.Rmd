...
troops_p <- ggplot(Minard.troops, aes(long, lat)) +
  geom_path(aes(linewidth = survivors, color = direction,
                group = group))
troops_p
```
...
ggplot(iris, aes(x = Sepal.Length, y = Petal.Length, color = Species)) +
  ...
  theme(
    ...
    panel.grid.minor = element_line(color = "gray80", linewidth = 0.1),
    panel.background = element_rect(fill = "white"),
    plot.background = element_rect(fill = "white"),
    panel.border = element_rect(color = "black", fill = NA, linewidth = 0.2),
    axis.title = element_text(size = 11),
    axis.text = element_text(size = 11, color = "black"),
    legend.background = element_rect(color = "gray30", linewidth = 0.2),
...
```{r}
# Your code here
carNames_to_label <- c("Mazda RX4","Mazda RX4 Wag","Datsun 710","Hornet 4 Drive")

#square brackets index row names
#grepl takes a text argument
mtcars2 %>% 
  rownames_to_column(var = "carType") %>% 
  filter(carType %in% carNames_to_label) %>% 
  ...
  labs (x = "Horsepower",
        y = "Miles Per Gallon")
#why did this work? how did I set only 4 of them?
...
```{r}
#YOUR CODE HERE
baseballPlot <- ggplot(DATA, aes(x = player_age, y = batting_average) +
                         facet_wrap()
)

#wrong exercise, skipped it
```
