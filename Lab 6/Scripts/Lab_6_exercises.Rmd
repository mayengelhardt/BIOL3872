...
as.Date("Jul/29/01", format = "%b/%d/%y")

as.Date("Dec-9-95", format = "%b-%d-%y")

as.POSIXct("December/4/13 19:11:31", format = "%B/%D/&y %h:%M:%S")

as.POSIXct("December-19-10 9:30:49pm", format = "")
...
```{r}
# YOUR CODE HERE
Bedford_ts$Date <- as.Date(Bedford_ts$Date, "%Y-%m-%d")
...
Bedford_Date <- as.Date(make_datetime(year=Bedford_ts$year,month=Bedford_ts$month, day=Bedford_ts$day))
