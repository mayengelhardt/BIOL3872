...
# a)
# YOUR CODE HERE
salamander_lengths = c(12.5, 14.2, 11.8, 13.6, 15.1, 12.9) #setting salamander length values from provided data
salamander_lengths

# b)
# YOUR CODE HERE
mean_length = mean(salamander_lengths) #getting the average of the above lengths and setting it to a variable
mean_length
```
:::
...
# d)
last_salamander = tail(salamander_lengths,1)
last_salamander
```
:::
...
field_samples = data.frame(
  sample_id = c("soo1","s002","s003","s004","s005"),
...
marine_survey = data.frame(
  ...
  salinity_ppt = c(35.2,35.8,36.1,35.3,36.5,35.9),
...
species_sd
# d)
deep_stations = marine_survey[marine_survey$depth_category == "deep",]
deep_stations

# e)
sorted_survey = sort(marine_survey$species_count,decreasing = TRUE)
sorted_survey
...
Very strong negative correlation - close to negative 1. THis means that with increasing depth, species count decreases in a very strongly correlated way (inverse relationship).
:::
