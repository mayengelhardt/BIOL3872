...
if(option1 > option2) {
  ...
  print("option1 and option2 are equal to one another.")
}
...
# A) 
marine_survey$environments<- with(marine_survey,ifelse(
  (species_count > quantile(species_count, probs=2/3)) &
...
```{r}
sheep <- c(seq(1, 100, 1))
counting_sheep <- numeric(length = 100)
...
numeric_summaries_df <- do.call(rbind, numeric_summaries)

# C) 
numeric_summaries_df$variables <- rownames(numeric_summaries_df)
...
total_abundance <- fish_abundance %>% 
  ...
  summarize(abundance_summ = sum(num_cpue, na.rm=TRUE), .groups="drop")
```
:::
